...
if (!$form->validate($_FILES)) {
    ...
    if ($profile_pic['error'] == UPLOAD_ERR_OK) {
        ...
        $name = basename($profile_pic['name']);
        $img_extension = pathinfo($profile_pic['name'], PATHINFO_EXTENSION);
    ...
    if ($uploaded) {
        ...
        // User_id exists, perform an update & Delete previous profile picture
        if ($result) {
